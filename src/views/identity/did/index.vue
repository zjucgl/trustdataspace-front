      ...
      <div style="height: calc(100vh - 260px)">
        ...
        >
          <el-table-column
            ...
            align="center"
          />
          <el-table-column prop="id" label="didID" align="center" />
          <el-table-column label="验证模式" align="center">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.verificationMethod">
                <span>
                  {{ item.id }}
                  <span v-if="index < scope.row.verificationMethod.length - 1">
                    、
                  </span>
                </span>
              </div>
          ...
          <el-table-column prop="service" label="服务名称" align="center">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.service">
                <div v-if="item.type === 'ProtocolEndpoint'">
                  {{ item.id }}
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="service" label="服务端点" align="center">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.service">
                <div v-if="item.type === 'ProtocolEndpoint'">
...
import { type DidData, getDidList } from "@/api/identity/did";
